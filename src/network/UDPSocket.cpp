...
bool UDPSocket::asyncSend(asio::ip::udp::endpoint p_destination,
  ...
  std::function<void(const asio::error_code &, std::size_t)> callback = [self, send_callback = std::move(p_callback)](const asio::error_code &p_error,
                                                                                                                      std::size_t) {
...
bool UDPSocket::asyncReceive(std::unique_ptr<UDPPacket> &p_rx_buffer,
  ...
  std::function<void(const asio::error_code &, std::size_t)> callback =
      [self, receive_callback = std::move(p_callback)](const asio::error_code &p_error, std::size_t p_len) {
        ...
        // Copy the source endpoint before another receive operation can overwrite it
        asio::ip::udp::endpoint receive_endpoint = self->m_receive_endpoint;
        ...
        if (receive_callback) {
          try {
            receive_callback(std::move(receive_packet), p_len, std::move(receive_endpoint), p_error);
          } catch (const std::exception &e) {
